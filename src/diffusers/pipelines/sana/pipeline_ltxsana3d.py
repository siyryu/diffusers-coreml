...
from ...models import SanaTransformer3DModel, AutoencoderKLLTXVideo
...
class LTXSana3dPipeline(DiffusionPipeline, SanaLoraLoaderMixin):
    r"""
    ...
    def __init__(
            ...
            transformer: SanaTransformer3DModel,
    ...
    def encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            # text_inputs.input_ids[0][0] = 32
            # text_inputs = torch.load("/data/baotang/sana_videodistill/video_distill/0331/caption_token.pt")
    ...
    def __call__(
        ...
        # prompt_embeds = torch.load("/data/baotang/sana_videodistill/video_distill/0331/caption_embs.pt").to(torch.float16)
        # negative_prompt_embeds = torch.load("/data/baotang/sana_videodistill/video_distill/0331/null_y.pt").to(torch.float16)
        ...
        if output_type == "latent":
            ...
            # )
            latents = (latents.detach() / self.vae.config.scaling_factor)
            ...
            image = self.vae.decode(latents.to(self.vae.dtype), timestep, return_dict=False)[0]
            # image = self.video_processor.postprocess_video(video, output_type=output_type)

            # latents = latents.to(self.vae.dtype)
            # try:
            #     B, C, D, H, W = latents.shape
            #     latents = latents.permute(0, 2, 1, 3, 4).reshape(-1, C, H, W)
            #     image = self.vae.decode(latents / self.vae.config.scaling_factor, return_dict=False)[0]
            #     image = image.reshape(B, D, image.shape[1], image.shape[2], image.shape[3]).permute(0, 2, 1, 3, 4)
