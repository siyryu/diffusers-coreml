...
class LTX3DSanaTransformerBlock(nn.Module):
    r"""
...
class LTXSanaTransformer3DModel(ModelMixin, ConfigMixin, PeftAdapterMixin):
    r"""
    ...
    #     pos_embed = self.get_1d_sincos_pos_embed(self.hidden_size, self.input_size[0] // self.patch_size, scale=1.0)
    #     pos_embed = torch.from_numpy(pos_embed).float().unsqueeze(0).requires_grad_(False)
    #     return pos_embed

    # def get_1d_sincos_pos_embed(self, embed_dim, length, scale=1.0):
